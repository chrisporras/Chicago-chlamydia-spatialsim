...
##Assign Gender and age calss by disrivutions
...
for(I in 1:length(ExtraData$Neighborhood)){ ###Each Neighborhood wants a certain number of married folks
...
beta = c(0.15, 0.2)
PI=numeric()
delta = 0.2  ####Percent that go for testing
deltavec = matrix(c(delta * exp(4 * (-(ClinicDist/max(ClinicDist))^2)), rep(delta, length(ExtraData$Neighborhood))), ncol = 2)
...
for (I in 1:length(ExtraData$Neighborhood)) {
  S[sample(which(NeighborhoodVec == I & ActualConnects > 0), ceiling(0.01 * length(which(NeighborhoodVec == I)))), 1] = 1
}
