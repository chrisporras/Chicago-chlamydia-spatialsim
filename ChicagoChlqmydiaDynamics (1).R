...
gender_vec=rep(NA, n) #vector that will contain each individual's gender

#Assigning each individual a gender
for (i in 1:n){
  theta=0.5 #50% chance of male or female
  gender_vec[i]<- rbinom(n = 1, 
            size = 1, 
            prob = theta) #0 indicates male, 1 indicates female
}

#Assigning each individual an age based on neighboorhood

for (i in 1:n){
  Neighboorhood=NeighborhoodVec[i]
  Age
}
...
PP=c(21.5,59.6,10.6/3, 10.6/3, 10.6/3, 5)/sum(c(21.5,59.6,10.6/3, 10.6/3, 10.6/3, 5)) #Distribution of partners
...
ggplot(Chicago2, aes(long, lat, group = group, fill = FRACT)) + geom_polygon() + coord_equal() +labs(x = "", y = "", fill = "Fraction Infected Per Community") +  scale_fill_gradientn(breaks= seq(0, max(FractionInfect), max(FractionInfect)/3), colours = c("lightblue", "steelblue3", 'royalblue4', "red") ) 
